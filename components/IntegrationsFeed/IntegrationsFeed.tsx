...
export function IntegrationsFeed({ className }: Props) {
  ...
  return (
    <div className={clsx(className, "flex flex-col gap-4 relative")}>
      <input
      ...
      <div className="flex flex-wrap gap-8 justify-center">
        {data
          ? data.items.map(({ fields, sys }) => (
              <div key={sys.id} className="p-10 border rounded-lg w-[315px]">
                {fields.title}
              </div>
            ))
      ...
      <nav className="sticky bottom-0 flex items-center justify-between border-t border-gray-200 px-4 sm:px-0">
        ...
        <div className="hidden md:-mt-px md:flex">
          {pages
            .slice(Math.max(0, currentIndex - 3), Math.max(currentIndex + 4, 7))
            .map((index) => (
              <span
                key={index}
                onClick={() =>
                  setParams((p) => ({ ...p, skip: index * limit }))
                }
                className={clsx(
...
function FeedItemSkeleton() {
  return (
    <div className="border border-gray-300 shadow rounded-lg p-10 w-[315px] min-h-[150px]">
